...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --set-env-vars="IM_PROJECT_ID=${PROJECT_ID},IM_LOCATION=${REGION},IM_SERVICE_ACCOUNT=projects/${PROJECT_ID}/serviceAccounts/${IM_EXEC_SA},DEFAULT_TARGET_PROJECT_ID=${TARGET_PROJECT_ID},TF_REPO_URL=https://github.com/sonmap/Gcp_Managed_GKE_GIT_ETC_04.git,TF_REPO_REF=main,TF_DIRECTORY=infra-manager/terraform/task-blueprint,FOUNDATION_TF_DIRECTORY=infra-manager/terraform/foundation,FOUNDATION_DEPLOYMENT_ID=foundation-analysis,GKE_PROJECT_ID=${PROJECT_ID},GKE_LOCATION=${REGION},GKE_CLUSTER_NAME=analysis-autopilot-a,GKE_NETWORK_NAME=managed02-dev-vpc,GKE_SUBNETWORK_NAME=managed02-dev-subnet,AUTO_CREATE_FOUNDATION=true"
...
echo
echo "PoC uses --allow-unauthenticated. Production should require authenticated/internal access."
